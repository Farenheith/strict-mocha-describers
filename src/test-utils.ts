...
export const testUtils = {
  prepare<T>(service: T, prototype: T, methodToTest?: keyof T) {
    ...
    const methods = testUtils.getMockableMethods<T>(
      subjects,
      service,
      methodToTest,
    );
  ...
  getMockableMethods<T>(
    subjects: T[],
    service: T,
    methodToTest: keyof T | undefined,
  ) {
    const methods: Array<keyof T> = [];
    for (const subject of subjects) {
      testUtils.getSubjectMockableMethods<T>(
        subject,
        service,
        methodToTest,
        methods,
  ...
  isMockable<T>(
    ...
    prototype: T,
    service: T,
    methodToTest?: keyof T,
  ) {
    return (
      key !== methodToTest &&
      typeof prototype[key] === 'function' &&
      // for instance methods
      ((service !== prototype && key !== 'constructor') ||
        // for static classes
        (service === prototype &&
          key !== 'apply' &&
          key !== 'bind' &&
  ...
  getMockedMethod<T>(name: keyof T): T[keyof T] {
    const ref = {
      [name]: () => {
        throw new Error(`${name} not mocked yet`);
      },
    };
    return ref[name as string] as any;
  },

  setupFunction<A extends Function, B extends Function>(
    transformer: (a: A) => B,
    mochaBase: BaseMochaType<A>,
  ) {
