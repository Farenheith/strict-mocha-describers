...
export class MethodDescribeHelper<
  Target,
  Class extends ClassOf<Target>
> extends StaticMethodDescribeHelper<Class> {
  constructor(protected readonly bootstrap: () => Target, cls: Class) {
    super(cls);
  }

  private createMethodDescribe = (
    suite: (title: string, fn: () => void) => void,
  ) => {
    const result = ((
      method: keyof Target,
