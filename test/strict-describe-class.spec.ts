...
describe('strict-describe-class.ts', () => {
  describe('mountClassDescribe()', () => {
    ...
    beforeEach(() => {
      ...
      sinon
        .stub(methodHelper, 'MethodDescribeHelper')
        .returns({ createDescribe });
      fn = sinon.stub();
    ...
    it('should return class Describer that uses MethodDescribeHelper.createDescribe', () => {
      ...
      expect(
        methodHelper.MethodDescribeHelper,
      ).to.have.been.calledOnceWithExactly('bootStrapFunction', Test);
  ...
  describe('mountStaticClassDescribe()', () => {
    ...
    beforeEach(() => {
      createStaticDescribe = sinon
        .stub()
        .returns('createStaticDescribe result' as any);
      sinon
        .stub(staticMethodHelper, 'StaticMethodDescribeHelper')
        .returns({ createStaticDescribe });
      fn = sinon.stub();
      suite = sinon.stub().callsFake((_description, callback) => callback());
    });
    ...
    it('should return static class Describer that uses MethodDescribeHelper.createStaticDescribe', () => {
      ...
      expect(
        staticMethodHelper.StaticMethodDescribeHelper,
      ).to.have.been.calledOnceWithExactly(Test);
      expect(createStaticDescribe).to.have.been.calledOnceWithExactly();
      expect(fn).to.have.been.calledOnceWithExactly(
        'createStaticDescribe result',
      );
      expect(result).to.be.undefined;
    });
  });
